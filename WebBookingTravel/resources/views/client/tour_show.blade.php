...
if (is_array($dec)) { $raw = $dec; } else { $raw = null; }
}
if (is_array($raw)) {
...
$u = null;
if (is_string($img)) { $u = $img; }
elseif (is_array($img)) { $u = $img['url'] ?? $img['src'] ?? $img['path'] ?? $img['image'] ?? $img['imageUrl'] ?? $img['image_url'] ?? null; }
if ($u) $gallery[] = $u;
}
}
// Always ensure primary image like list
$primary = $tour->image_path ?: asset('assets/images/destinations/dest1.jpg');
if (!$gallery) { $gallery[] = $primary; }
else if (!in_array($primary, $gallery, true)) { array_unshift($gallery, $primary); }

$typeVal = $tour->type ?? ($tour->tourType ?? null);
$typeLabel = $typeVal ? (strtolower(trim($typeVal))==='international' ? 'Nước ngoài' : (strtolower(trim($typeVal))==='domestic' ? 'Trong nước' : $typeVal)) : null;
...
<div class="container py-4">
    ...
    <div class="row g-4">
        <div class="col-lg-6">
            <div class="border rounded-3 overflow-hidden bg-light hero-slideshow" data-autoplay="5000">
                ...
                <div class="hero-slide {{ $i===0 ? 'is-active' : '' }}" aria-hidden="{{ $i===0 ? 'false' : 'true' }}">
                    <img src="{{ $src }}" alt="{{ $tour->title }}" class="w-100" style="aspect-ratio: 16/10; object-fit: cover;">
                </div>
                @endforeach
                @if(count($gallery) > 1)
                <button type="button" class="slide-nav prev btn btn-light btn-sm" aria-label="Prev"><i class="far fa-chevron-left"></i></button>
                <button type="button" class="slide-nav next btn btn-light btn-sm" aria-label="Next"><i class="far fa-chevron-right"></i></button>
                <div class="d-flex justify-content-center gap-2 py-2">
                    @foreach($gallery as $i => $u)
                    <button type="button" class="slide-dot {{ $i===0 ? 'active' : '' }} btn btn-xs btn-secondary" style="width:8px;height:8px;border-radius:50%;padding:0;border:none;"></button>
                    @endforeach
                </div>
                @endif
            </div>
        ...
        <div class="col-lg-6">
            ...
            <div class="d-flex flex-wrap gap-2 mb-3">
                ...
                @endif
                @if(!empty($tour->days))
                <span class="badge bg-light text-dark"><i class="far fa-clock me-1"></i>{{ $tour->days }} ngày</span>
                @endif
            </div>

            <div class="border rounded-3 p-3 mb-3">
                @if(!is_null($adult))
                <div class="fs-5 mb-1">Từ <strong>{{ number_format($adult, 0, ',', '.') }}đ</strong> <span class="text-muted">/ người lớn</span></div>
                @elseif(!is_null($child))
                <div class="fs-5 mb-1">Từ <strong>{{ number_format($child, 0, ',', '.') }}đ</strong> <span class="text-muted">/ trẻ em</span></div>
                @else
            ...
            <div class="row g-2 mb-3">
                ...
                <div class="col-sm-6"><i class="far fa-map-marker-alt me-1"></i><strong>Khởi hành:</strong> {{ $tour->departurePoint }}</div>
                @endif
                @if(!empty($tour->pickupPoint))
                <div class="col-sm-6"><i class="far fa-bus me-1"></i><strong>Điểm đón:</strong> {{ $tour->pickupPoint }}</div>
                @endif
            </div>
    ...
    if (json_last_error() === JSON_ERROR_NONE) { $pricesJson = $tmp; }
    }
    $itineraryJson = $tour->itinerary;
    if (is_string($itineraryJson)) {
    $tmp = json_decode($itineraryJson, true);
    if (json_last_error() === JSON_ERROR_NONE) { $itineraryJson = $tmp; }
    }
    ...
    <div class="mt-4">
        ...
        <div class="section-card p-3">
            <div class="row g-3">
                <div class="col-md-4">
                    <div class="text-muted">Mã tour</div>
                    <div class="fw-semibold">{{ $tour->tourID }}</div>
                </div>
                @if(!empty($tour->departurePoint))
                <div class="col-md-4">
    ...
    <div class="mt-4">
        ...
        <div class="section-card p-3">
            ...
            <div>Người lớn: <strong>{{ number_format((float)($tour->priceAdult ?? 0), 0, ',', '.') }}đ</strong></div>
    ...
    <div class="mt-4">
        ...
        <div class="section-card p-3">
            ...
            <div class="vstack">
                ...
                @php
                $dTitle = is_array($day) ? ($day['title'] ?? ($day['name'] ?? ('Ngày ' . ($day['day'] ?? ($idx+1))))) : ('Ngày ' . ($idx+1));
                $dDesc = is_array($day) ? ($day['content'] ?? ($day['description'] ?? (isset($day['details']) ? (is_string($day['details']) ? $day['details'] : json_encode($day['details'], JSON_UNESCAPED_UNICODE)) : ''))) : (string) $day;
                $dTime = is_array($day) ? ($day['time'] ?? (isset($day['startTime'],$day['endTime']) ? ($day['startTime'].' - '.$day['endTime']) : null)) : null;
                $dTransport = is_array($day) ? ($day['transport'] ?? $day['vehicle'] ?? $day['bus'] ?? $day['train'] ?? null) : null;
                $dDistance = is_array($day) ? ($day['distance'] ?? null) : null;
                $dPlaces = is_array($day) ? ($day['places'] ?? $day['locations'] ?? $day['stops'] ?? null) : null;
                $dItems = is_array($day) ? ($day['items'] ?? $day['activities'] ?? ($day['activity'] ?? null)) : null;
                $dMealsRaw = is_array($day) ? ($day['meals'] ?? $day['meal'] ?? null) : null;
                $dImgs = is_array($day) ? ($day['images'] ?? $day['photos'] ?? null) : null;
                $meals = [];
                if (is_array($dMealsRaw)) { $meals = $dMealsRaw; }
                elseif (is_string($dMealsRaw)) { $meals = array_filter(array_map('trim', preg_split('/[,;\/]+/u', $dMealsRaw))); }
                else {
                if (is_array($day)) {
                foreach (['breakfast'=>'Sáng','lunch'=>'Trưa','dinner'=>'Tối'] as $mk=>$ml) {
                if (!empty($day[$mk])) $meals[] = $ml;
                }
                }
                }
                $dGuide = is_array($day) ? ($day['guide'] ?? $day['tourGuide'] ?? null) : null;
                $dNotes = is_array($day) ? ($day['note'] ?? $day['notes'] ?? null) : null;
                if (!empty($dPlaces) && is_string($dPlaces)) { $dPlaces = array_filter(array_map('trim', preg_split('/[,;]+/u', $dPlaces))); }
                if (!empty($dItems) && is_string($dItems)) { $dItems = array_filter(array_map('trim', preg_split('/\n|[,;]+/u', $dItems))); }
                $thumbs = [];
                if (is_array($dImgs)) {
                foreach ($dImgs as $im) {
                $url = is_string($im) ? $im : (is_array($im) ? ($im['url'] ?? $im['src'] ?? $im['path'] ?? null) : null);
                if ($url) $thumbs[] = (str_starts_with($url,'http') || str_starts_with($url,'//')) ? $url : asset(ltrim(str_replace('\\','/',$url), '/'));
                }
                }
                @endphp
                <div class="itinerary-day">
                    <div class="d-flex align-items-start justify-content-between gap-2">
                        <div>
                            ...
                            @if($dTime)
                            <div class="icon-text muted small"><i class="far fa-clock"></i><span>{{ $dTime }}</span></div>
                            @endif
                            @if($dTransport)
                            <div class="icon-text muted small"><i class="far fa-bus"></i><span>{{ $dTransport }}</span></div>
                            @endif
                            @if($dDistance)
                            <div class="icon-text muted small"><i class="far fa-road"></i><span>{{ $dDistance }}</span></div>
                            @endif
                            @if(!empty($dPlaces) && is_array($dPlaces))
                            <div class="muted small mt-1"><i class="far fa-map-marker-alt me-1"></i>{{ implode(' • ', $dPlaces) }}</div>
                            @endif
                        </div>
                        @if(!empty($meals))
                    ...
                    @endif
                    @if(!empty($dGuide))
                    <div class="muted small mt-1"><i class="far fa-user me-1"></i>Hướng dẫn viên: {{ is_string($dGuide) ? $dGuide : json_encode($dGuide, JSON_UNESCAPED_UNICODE) }}</div>
                    @endif
                    @if(!empty($dNotes))
                    <div class="muted small mt-1"><i class="far fa-info-circle me-1"></i>{{ is_string($dNotes) ? $dNotes : json_encode($dNotes, JSON_UNESCAPED_UNICODE) }}</div>
                    @endif
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-2">
                        @foreach($thumbs as $t)
                        <img src="{{ $t }}" alt="{{ $dTitle }}" style="width:88px;height:66px;object-fit:cover;border-radius:.5rem;border:1px solid rgba(0,0,0,.08);" />
                        @endforeach
                    </div>
                    @endif
                </div>
    ...
    <div class="mt-4">
        ...
        <div class="section-card p-3">
            ...
            <div class="row g-3">
                ...
                $hRooms = is_array($h) && isset($h['rooms']) && is_array($h['rooms']) ? $h['rooms'] : [];
                $hImg = null;
                if (is_array($h)) {
                $hImg = $h['image'] ?? $h['imageUrl'] ?? $h['image_url'] ?? $h['photo'] ?? null;
                if (!$hImg && !empty($h['images']) && is_array($h['images'])) {
                $hImg = is_string($h['images'][0] ?? null) ? $h['images'][0] : ( (is_array($h['images'][0] ?? null) ? ($h['images'][0]['url'] ?? null) : null) );
                }
                }
                $imgSrc = null;
                if ($hImg) {
                ...
                // Price extraction
                $hPrice = null; $hPriceLabel = 'đ'; $nightly = false;
                if (is_array($h)) {
                foreach (['price','pricePerNight','per_night','nightly','roomPrice','minPrice','cost','rate'] as $pk) {
                if (isset($h[$pk]) && is_numeric($h[$pk])) { $hPrice = (float)$h[$pk]; break; }
                if (isset($h[$pk]) && is_string($h[$pk]) && is_numeric(preg_replace('/[^0-9.]/','',$h[$pk]))) { $hPrice = (float)preg_replace('/[^0-9.]/','',$h[$pk]); break; }
                }
                if (!$hPrice && !empty($h['rates']) && is_array($h['rates'])) {
                $min = null;
                foreach ($h['rates'] as $r) {
                $pv = is_array($r) ? ($r['price'] ?? $r['amount'] ?? null) : (is_numeric($r) ? $r : null);
                if (is_string($pv)) $pv = (float)preg_replace('/[^0-9.]/','',$pv);
                if (is_numeric($pv)) { $min = is_null($min) ? (float)$pv : min($min, (float)$pv); }
                }
                if (!is_null($min)) $hPrice = $min;
                }
                foreach (['pricePerNight','per_night','nightly'] as $nk) { if (isset($h[$nk])) { $nightly = true; break; } }
                if (!empty($h['currency']) && is_string($h['currency']) && strtoupper($h['currency']) !== 'VND') {
                ...
                $amenities = [];
                if (is_array($h)) {
                if (!empty($h['amenities']) && is_array($h['amenities'])) $amenities = $h['amenities'];
                elseif (!empty($h['features']) && is_array($h['features'])) $amenities = $h['features'];
                }
                @endphp
                <div class="col-md-6">
                    <div class="border rounded-3 p-2 h-100 d-flex gap-2 align-items-start">
                        @if($imgSrc)
                        <img src="{{ $imgSrc }}" alt="{{ $hName }}" style="width:84px;height:84px;object-fit:cover;border-radius:.5rem;" />
                        @endif
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                <div class="fw-semibold">{{ $hName }}</div>
                                @if($hStars) <div class="text-warning small">({{ $hStars }}★)</div> @endif
                                @if(!is_null($hPrice))<span class="price-chip">{{ number_format($hPrice, 0, ',', '.') }}{{ $hPriceLabel }}@if($nightly) / đêm @endif</span>@endif
                            </div>
                            @if($hAddr)<div class="text-muted small mt-1">{{ $hAddr }}</div>@endif
                            @if(!empty($amenities))
                            <div class="d-flex flex-wrap gap-1 mt-2">
                                @foreach(array_slice($amenities,0,6) as $am)
                                <span class="amenity">{{ is_string($am) ? $am : json_encode($am, JSON_UNESCAPED_UNICODE) }}</span>
                                @endforeach
                            </div>
                            @endif
                            @if(!empty($hRooms))
                            <div class="d-flex flex-wrap gap-1 mt-2">
                                ...
                                <span class="amenity">{{ ucfirst($rk) }}: {{ number_format($roomPrice, 0, ',', '.') }}đ</span>
                                @else
                                <span class="amenity">{{ ucfirst($rk) }}: {{ is_string($rv) ? $rv : json_encode($rv, JSON_UNESCAPED_UNICODE) }}</span>
                                @endif
                                @endforeach
                            </div>
                            @endif
